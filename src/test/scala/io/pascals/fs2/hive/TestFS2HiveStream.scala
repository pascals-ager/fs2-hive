...
import io.pascals.fs2.hive.tags.{
  IntegrationTest,
  InvalidTableTest,
...
class TestFS2HiveStream extends FunSuite with Matchers {
  ...
  test("Simple Streaming Connection Test", IntegrationTest) {
    ...
    val builder: HiveStreamingConnection.Builder = HiveStreamingConnection
      .newBuilder()
      .withDatabase(dbName)
      .withTable(tblName)
      .withAgentInfo("hive")
    ...
    val s: Stream[IO, Unit] = Stream
      .bracket(IO.delay(builder.connect()))(conn => IO.delay(conn.close()))
      .map { conn =>
        {
  ...
  test("InvalidTable Exception Test", InvalidTableTest) {
    ...
    f.handleErrorWith { f =>
      Stream {
        ...
        assert(
          f.getLocalizedMessage == "Invalid table db:test, table:simple_alerts: hive.test.simple_alerts table not found"
        )
      }
    }.compile.drain.unsafeRunSync()
  }
  ...
  test("InvocationTargetException Test", InvocationTargetExceptionTest) {
    ...
    f.handleErrorWith { f =>
      Stream.emit {
        ...
        assert(
          f.getCause.isInstanceOf[java.lang.reflect.InvocationTargetException]
        )
        assert(
          f.getLocalizedMessage == "Unable to instantiate org.apache.hadoop.hive.metastore.HiveMetaStoreClient"
        )
      }
    }.compile.drain.unsafeRunSync()
  }
